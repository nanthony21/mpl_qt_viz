...
class DockablePlotWindow(QMainWindow):
    _DEFAULT_FIG_SIZE = (
        6.4,
        4.8,
    )  # Got this from https://matplotlib.org/api/_as_gen/matplotlib.figure.Figure.html#matplotlib-figure-figure
    _DOCKAREAMAP = {
    ...
    def __init__(self, title: str = "Dockable Plots"):
        ...
        self._dockedWidgets: t_.List[
            GenericDockableWidget
        ] = []  # List of all add dock widgets that are not plots.
        self.setDockOptions(
    ...
    def _pdfSaveAction(self):
        ...
        fName, filterUsed = QFileDialog.getSaveFileName(
            self, "Save PDF", os.path.expanduser("~"), "PDF (*.pdf)"
        )
    ...
    def saveToPDF(self, filePath: str):  # TODO make this callable from a Qt menubar
        ...
        with PdfPages(filePath) as pdf:
            for k, fig in self.figures.items():
                ...
                fig.set_size_inches(
                    self._DEFAULT_FIG_SIZE
                )  # Set to the original figure size for saving.
