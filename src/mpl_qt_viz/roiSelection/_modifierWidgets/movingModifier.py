...
class MovingModifier(ModifierWidgetBase):
    ...
    def __init__(
        ...
        self.indicatorLine = Line2D(
            [0, 0], [0, 0], color="k", linestyle="dashed", animated=True
        )
    ...
    def initialize(self, setOfVerts: typing.Sequence[ModifierWidgetBase.PolygonCoords]):
        ...
        for verts in setOfVerts:
            poly = Polygon(
                verts,
                facecolor=(0.7, 0.3, 0, 0.1),
                linewidth=2,
                linestyle="dotted",
                edgecolor=(1, 0, 0, 0.9),
                animated=True,
            )  # Having animated true here helps with rendering.
            self.addArtist(poly)
    ...
    def _ondrag(self, event: MouseEvent):
        ...
        if "shift" in self.state:
            ...
            delta = (
                mousePoint - self.initialClickPoint
            )  # Difference between current mouse position and initial pos
            self.indicatorLine.set_data(
                [
                    [self.initialClickPoint[0], event.xdata],
                    [self.initialClickPoint[1], event.ydata],
                ]
            )
            self._setTranslation(self.initialTranslation + delta)
            self._setRotationPoint(mousePoint)

        self.transformText.set_text(
            f"Trans: ({self.translation[0]:.1f}, {self.translation[1]:.1f})\nRot: {np.degrees(self.angle):.1f} deg."
        )
    ...
    def _updatePolygons(self):
        ...
        for coords, poly in zip(self.originalCoords, self.polygonArtists):
            ...
            coords = np.array(coords) - (
                self.rotationPivotPoint - self.translation
            )  # So that any rotation is centered around our initial click position
    ...
    def _setRotationPoint(self, point: typing.Tuple[float, float]):
        ...
        self.angleRefLine.set_data(
            [point[0], point[0] + self.lineMagnitude], [point[1], point[1]]
        )
    ...
    def _setRotation(self, radians: float):
        ...
        self.angleIndicatorLine.set_data(
            [
                self.rotationPivotPoint[0],
                self.rotationPivotPoint[0] + np.cos(self.angle) * self.lineMagnitude,
            ],
            [
                self.rotationPivotPoint[1],
                self.rotationPivotPoint[1] + np.sin(self.angle) * self.lineMagnitude,
            ],
        )
